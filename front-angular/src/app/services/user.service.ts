...
export class UserService {
  private API_URL = `${environment.apiUrl}${environment.endpoints.users}`;
  private AUTH_URL = `${environment.apiUrl}${environment.endpoints.auth}`;
  ...
  login(email: string, password: string): Observable<any> {
    return this.http.post<any>(`${this.AUTH_URL}`, { email, password });
  }
  ...
  createUser(user: Partial<UserInterface>): Observable<UserInterface> {
    return this.http.post<UserInterface>(this.API_URL, user, { headers: this.getAuthHeaders() });
  }
